...
def register():
    ...
    if not name or not age or not email or not occupation:
        messagebox.showerror("Input Error", "Please fill in all fields.")
        return

    # Simulate saving the data (you can add database functionality here)
    messagebox.showinfo("Success", f"Registration successful for {name} as a {occupation}!")
...
# Occupation dropdown
label_occupation = tk.Label(root, text="Occupation:")
label_occupation.grid(row=4, column=0, padx=10, pady=5)

occupation_var = tk.StringVar(value="Select Occupation")
occupation_options = ["Student", "Employee", "Other"]
occupation_menu = tk.OptionMenu(root, occupation_var, *occupation_options)
occupation_menu.grid(row=4, column=1, padx=10, pady=5)
